...
DELIVERY-1: Keep the upstream feature PR focused on runtime code, the setting, dictionary-author documentation, tests, and the translator benchmark. These files are needed to review and maintain the feature.

DELIVERY-2: Keep the research report, raw evidence, demo dictionary, and this working plan outside the upstream feature diff. Publish those review aids from the fork's report branch and link them from the PR description. This avoids adding generated evidence and a temporary plan to the product repository.

DELIVERY-3: Preserve report source on the Pages branch. The report must use interactive plots, show the underlying tables, explain several real use cases, and provide the demo dictionary as a direct download.
