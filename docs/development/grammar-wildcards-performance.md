...
[F-1. Explore the interactive total lookup-time plot.](grammar-wildcards-performance/report.html#F-1) Hover a line to see the exact value. The report keeps the source table under the plot.
...
[F-2. Explore the interactive added-time plot.](grammar-wildcards-performance/report.html#F-2) Orange bars mark the heavier 50-row and 100-row sensitivity checks. Bar positions are categories, not equal numeric intervals.
...
[F-3 and F-4. Explore the interactive scan-length and definition-size plots.](grammar-wildcards-performance/report.html#F-3) The scan test truncates one natural Japanese sentence. The definition-size test holds the input and pattern count fixed.
...
R-21. Reproduction: evidence.zip contains measure.mjs, generate.py, raw results with every timing sample, summary.csv, and the earlier stress results. From the repository root, unzip the evidence into a temporary directory, run node /path/to/measure.mjs /path/to/jmdict_english.zip, then python3 /path/to/generate.py /path/to/output. The harness needs this checkout's Node dependencies and Playwright Chromium. The older plot script needs Python with matplotlib and numpy, but the published report uses Chart.js and real HTML tables. The JMdict fixture has revision jmdict4 and SHA-256 2583ad84ffc446122230aace230a8c5979751aaf19b28593ab7853c04a48e4f5. The dictionary itself is not bundled.

R-22. Artifacts: [interactive HTML report](grammar-wildcards-performance/report.html), [demo dictionary](grammar-wildcards-performance/grammar-wildcards-demo.zip), [CSV data](grammar-wildcards-performance/summary.csv), and [reproduction bundle](grammar-wildcards-performance/evidence.zip).
